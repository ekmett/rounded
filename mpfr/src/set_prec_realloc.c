/* mpfr_set_prec_realloc -- reset the precision of a floating-point number
...
/* forgetting this include caused a really annoying and hard-to-track bug.
 * can you guess waht it was? :)
 */
#include <stdlib.h> 

void
mpfr_set_prec_realloc (mpfr_ptr x, mpfr_prec_t p)
{
  ...
  if (xsize > xoldsize)
    {
      tmp = (mpfr_limb_ptr) realloc
        (MPFR_GET_REAL_PTR(x), MPFR_MALLOC_SIZE(xsize));
